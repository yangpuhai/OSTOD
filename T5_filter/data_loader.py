        ...
        for dial_idx, dial_dict in dials.items():
            ...
            for ti, turn in enumerate(dial_dict["log"]):
                ...
                for key , values in system_act.items():
                    ...
                    if name_list != []:
                        act_list.append((actinfo + " ( " + " ".join([name for name in name_list]) + " )").lower())
                    else:
                        act_list.append((actinfo).lower())

                act_str = " , ".join(act_list)
                input_text = "[usr] " + turn["user"] + f" {tokenizer.sep_token}" + " [sys] " + turn["resp_nodelx"] + f" {tokenizer.eos_token}"
                if args["history"] != 0:
                    if dialogue_history != []:
                        input_text = ' '.join(dialogue_history[-args["history"]:]) + " [usr] " + turn["user"] + f" {tokenizer.sep_token}" + " [sys] " + turn["resp_nodelx"] + f" {tokenizer.eos_token}"

                output_text = act_str + f" {tokenizer.eos_token}"
